...
export async function signAccessToken(payload: Omit<TokenPayload, 'iss' | 'iat'>): Promise<string> {
...
export function setAuthCookies(
  ...
  res.headers.append(
    'Set-Cookie',
    `access_token=${accessToken}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${accessMaxAge}${isProd ? '; Secure' : ''}`
  )
  res.headers.append(
    'Set-Cookie',
    `refresh_token=${refreshToken}; HttpOnly; SameSite=Strict; Path=/api/auth; Max-Age=${refreshMaxAge}${isProd ? '; Secure' : ''}`
  )
}

export function clearAuthCookies(res: Response): void {
  res.headers.append('Set-Cookie', 'access_token=; HttpOnly; Path=/; Max-Age=0')
  res.headers.append('Set-Cookie', 'refresh_token=; HttpOnly; Path=/api/auth; Max-Age=0')
}